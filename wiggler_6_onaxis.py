# -*- coding: utf-8 -*-
"""
Created on Thu Feb 23 16:31:22 2023
...
endmagnetSep = magnetSep*2 #quarter strength, to account for keeping beam on axis and average displacement 0
penmagnetSep = magnetSep*2/np.sqrt(3) #3/4 strength to account for keeping beam on axis and average displacement 0
#NB magnets follow inverse square law 
magnetWidth = 150 #vertically
endmagnetWidth = magnetWidth-endmagnetSep+magnetSep
penmagnetWidth = magnetWidth-penmagnetSep+magnetSep
...
numrepeats= 39#41 total, but minus 2 for ends so 39 gives pDR spec
mess_size=0 #set to zero for automatic
margin=0 #if you make this non-zero will have to tweak some things
boundhoriz=700
oddmagnetLength = Ldrift-2*interMagnet
print('oddmagnetlength is', oddmagnetLength, 'mm')
#left_edge=-1.5*magnetLength-2*interMagnet-
og_thickness=2*magnetLength+2*oddmagnetLength+4*interMagnet
lengthtotal=og_thickness*(numrepeats+3)+oddmagnetLength
print('Total length is', lengthtotal/1000, 'm')
print('End magnet width is', endmagnetWidth, 'mm')# want this to be 100mm min for maximum field
print('Penultimate magnet width is', penmagnetWidth, 'mm')
#-----------------------------------------------------------------------------------------------
...
femm.mi_addnode(-boundhoriz,-4*magnetWidth)
femm.mi_addnode(og_thickness*(numrepeats+3)+2*margin+boundhoriz,4*magnetWidth)
femm.mi_addnode(og_thickness*(numrepeats+3)+2*margin+boundhoriz,-4*magnetWidth)
femm.mi_drawline(-boundhoriz,4*magnetWidth,-boundhoriz,-4*magnetWidth) #left boundary
femm.mi_drawline(og_thickness*(numrepeats+3)+2*margin+boundhoriz,4*magnetWidth,
                 og_thickness*(numrepeats+3)+2*margin+boundhoriz,-4*magnetWidth) #right boundary
femm.mi_drawline(-boundhoriz,4*magnetWidth,og_thickness*(numrepeats+3)+2*margin+boundhoriz,4*magnetWidth) #top boundary
femm.mi_drawline(-boundhoriz,-4*magnetWidth,og_thickness*(numrepeats+3)+2*margin+boundhoriz,-4*magnetWidth) #bottom boundary
...
drawoddMagnet(oddmagnetLength/2+interMagnet+margin+og_thickness,magnetWidth/2+magnetSep,oddmagnetLength, magnetWidth)
drawMagnet(magnetLength/2+oddmagnetLength+2*interMagnet+margin+og_thickness,magnetWidth/2+magnetSep,magnetLength, magnetWidth)
drawoddMagnet(oddmagnetLength/2+magnetLength+oddmagnetLength+3*interMagnet+margin+og_thickness,
            magnetWidth/2+magnetSep,oddmagnetLength, magnetWidth)
drawMagnet(magnetLength/2+oddmagnetLength+magnetLength+oddmagnetLength+4*interMagnet+margin+og_thickness,
...
femm.mi_addblocklabel(1.5*magnetLength+2*oddmagnetLength+4*interMagnet+margin+og_thickness,magnetSep+magnetWidth/2)#4th
femm.mi_selectlabel(1.5*magnetLength+2*oddmagnetLength+4*interMagnet+margin+og_thickness,magnetSep+magnetWidth/2)
femm.mi_setblockprop(magnetType, 0, mess_size, "", 90, 0, 0)
femm.mi_clearselected()

femm.mi_addblocklabel(1*magnetLength+1.5*oddmagnetLength+3*interMagnet+margin+og_thickness,magnetSep+magnetWidth/2)#3rd
femm.mi_selectlabel(1*magnetLength+1.5*oddmagnetLength+3*interMagnet+margin+og_thickness,magnetSep+magnetWidth/2)
femm.mi_setblockprop(magnetType, 0, mess_size, "", 180, 0, 0)
femm.mi_clearselected()

femm.mi_addblocklabel(0.5*magnetLength+1*oddmagnetLength+2*interMagnet+margin+og_thickness,magnetSep+magnetWidth/2) #2nd
femm.mi_selectlabel(0.5*magnetLength+1*oddmagnetLength+2*interMagnet+margin+og_thickness,magnetSep+magnetWidth/2)
femm.mi_setblockprop(magnetType, 0, mess_size, "", -90, 0, 0)
femm.mi_clearselected()

femm.mi_addblocklabel(0.5*oddmagnetLength+interMagnet+margin+og_thickness,magnetSep+magnetWidth/2) #1st
femm.mi_selectlabel(0.5*oddmagnetLength+interMagnet+margin+og_thickness,magnetSep+magnetWidth/2)
...
femm.mi_selectrectangle(0+margin+og_thickness,magnetSep+magnetWidth+steelThick,
                        2*magnetLength+2*oddmagnetLength+4*interMagnet+margin+og_thickness,-magnetSep-magnetWidth-steelThick, 4)
femm.mi_copytranslate2(og_thickness, 0, numrepeats, 4)
femm.mi_clearselected()

#adding end magnets to make beam on axis and mean displacement 0.

drawoddMagnet(oddmagnetLength/2+interMagnet+margin,endmagnetWidth/2+endmagnetSep,oddmagnetLength, endmagnetWidth)
drawMagnet(magnetLength/2+oddmagnetLength+2*interMagnet+margin,endmagnetWidth/2+endmagnetSep,magnetLength, endmagnetWidth)

drawoddMagnet(oddmagnetLength/2+magnetLength+oddmagnetLength+3*interMagnet+margin,
            penmagnetWidth/2+penmagnetSep,oddmagnetLength, penmagnetWidth)
drawMagnet(magnetLength/2+oddmagnetLength+magnetLength+oddmagnetLength+4*interMagnet+margin,
            penmagnetWidth/2+penmagnetSep,magnetLength, penmagnetWidth)
...
#adding material properties to end magnet cell
#Function attributes: mi_setblockprop('blockname', automesh, meshsize, 'incircuit', magdir, group, turns)
#Top row 
femm.mi_clearselected()
...
femm.mi_selectrectangle(0,magnetSep+magnetWidth+steelThick,
                        2*magnetLength+3*oddmagnetLength+5*interMagnet+margin,-magnetSep-magnetWidth-steelThick, 4)
#mi_mirror(x1,y1,x2,y2) mirror the selected objects about a line passing through the points (x1,y1) and (x2,y2).
femm.mi_copytranslate2(lengthtotal-og_thickness-oddmagnetLength,0,1,2)
femm.mi_selectrectangle(0,magnetSep+magnetWidth+steelThick,
                        2*magnetLength+3*oddmagnetLength+5*interMagnet+margin,-magnetSep-magnetWidth-steelThick, 4)
femm.mi_mirror2(lengthtotal/2, -magnetSep+magnetWidth+steelThick, lengthtotal/2, magnetSep+magnetWidth+steelThick, 1)
femm.mi_clearselected()

# draw steel sheet

femm.mi_drawline(margin,magnetSep+magnetWidth+steelThick,
(numrepeats+3)*(2*magnetLength+2*oddmagnetLength+4*interMagnet)+2*margin+oddmagnetLength,magnetSep+magnetWidth+steelThick)#outer long line
femm.mi_drawline((numrepeats+3)*(2*magnetLength+2*oddmagnetLength+4*interMagnet)+2*margin+oddmagnetLength,magnetSep+magnetWidth+steelThick,
  (numrepeats+3)*(2*magnetLength+2*oddmagnetLength+4*interMagnet)+2*margin+oddmagnetLength,magnetSep+magnetWidth)

femm.mi_drawline(margin,magnetSep+magnetWidth,
(numrepeats+3)*(2*magnetLength+2*oddmagnetLength+4*interMagnet)+2*margin+oddmagnetLength,magnetSep+magnetWidth)#inner long line
femm.mi_drawline(margin,-magnetSep-magnetWidth,
(numrepeats+3)*(2*magnetLength+2*oddmagnetLength+4*interMagnet)+2*margin+oddmagnetLength,-magnetSep-magnetWidth)  

femm.mi_drawline(margin,-magnetSep-magnetWidth-steelThick,
(numrepeats+3)*(2*magnetLength+2*oddmagnetLength+4*interMagnet)+2*margin+oddmagnetLength,-magnetSep-magnetWidth-steelThick)  
femm.mi_drawline((numrepeats+3)*(2*magnetLength+2*oddmagnetLength+4*interMagnet)+2*margin+oddmagnetLength,-magnetSep-magnetWidth-steelThick,
(numrepeats+3)*(2*magnetLength+2*oddmagnetLength+4*interMagnet)+2*margin+oddmagnetLength,-magnetSep-magnetWidth)
...
femm.mi_addnode(0,0)
femm.mi_addnode(og_thickness*(numrepeats+3)+2*margin+oddmagnetLength,0)
femm.mi_clearselected()
...
numpoints=25000
# points = np.linspace(0, og_thickness*(numrepeats+3)+2*margin+oddmagnetLength, numpoints)
# X,Y = np.meshgrid(points,points)
#femm.mo_seteditmode('contour')
Blist = []
xlist=[]
femm.mo_addcontour(0, 0)
femm.mo_addcontour(og_thickness*(numrepeats+3)+2*margin+oddmagnetLength,0)
